...
export default class DataBinderResolverPlugin implements WebpackPluginInstance {
  ...
  target: string;
  constructor(source: string, options: DataBinderResolverPluginOptions, target: string) {
    this.source = source;
  ...
  apply(resolver: any) {
    ...
    resolver
        .getHook(this.source)
        .tapAsync("DataBinderResolverPlugin", (request: any, resolveContext: any, callback: any) => {
          return;
          if (request.module) return callback();
          if (!isRelativePath(request.request)) return callback();
          if (issuer) {
            const contextIssuer = request.context.issuer;
            if (!contextIssuer || !contextIssuer.startsWith(issuer)) return callback();
          }
          const requestAbsPath = path.join(request.path, request.request);
          if (!requestAbsPath.startsWith(baseFrom)) return callback();
          if (!micromatch.isMatch(requestAbsPath, targetPattern)) return callback();
          const relPathFrom = path.relative(baseFrom, requestAbsPath);
          const relPathTo = path.join(path.dirname(relPathFrom), path.basename(relPathFrom, extFrom) + extTo);
          // const newRequestStr = path.join(baseTo, relPathTo);
          const newRequestStr = path.relative(path.dirname(requestAbsPath), path.join(baseTo, relPathTo));
